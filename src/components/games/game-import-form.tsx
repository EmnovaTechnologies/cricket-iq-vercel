...
import { importGamesAction } from '@/lib/actions/import-actions'; // Updated import path
import type { CsvGameImportRow, GameImportResult } from '@/types';
import { Loader2, Upload, AlertTriangle, CheckCircle, ListChecks } from 'lucide-react';
...
export function GameImportForm() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Simulate progress for client-side feedback, actual progress happens server-side
      // In a real scenario with large files, you might chunk uploads or use websockets for progress
      ...
      if (result.success) {
        toast({
          ...
          description: `${result.successfulImports} games imported successfully. ${result.failedImports} games failed.`,
        });
      } else {
        toast({
    ...
    <div className="space-y-6">
      ...
      >
        {isLoading ? (
          ...
          <>
            <Upload className="mr-2 h-4 w-4" /> Import Games ({parsedData.length} rows)
          </>
        )}
      </Button>
          ...
          <CardContent className="space-y-4">
            {importResult.success ? (
              <div className="flex items-center gap-2 text-green-600">
                ...
                <span>{importResult.message || 'Import process completed.'}</span>
              </div>
            ) : (
              <div className="flex items-center gap-2 text-destructive">
                <AlertTriangle className="h-5 w-5" />
                <span>{importResult.message || 'Import process encountered errors.'}</span>
              </div>
            )}
            <p>Successfully imported: <strong className="text-green-600">{importResult.successfulImports}</strong> games.</p>
            <p>Failed to import: <strong className="text-destructive">{importResult.failedImports}</strong> games.</p>
