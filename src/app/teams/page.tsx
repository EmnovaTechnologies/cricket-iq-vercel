...
export default function TeamsPage() {
  const { activeOrganizationId, activeOrganizationDetails, loading: authLoading, isOrgLoading, effectivePermissions, isPermissionsLoading, userProfile } = useAuth();
  ...
  const fetchTeams = useCallback(async () => {
    ...
    try {
      ...
      const canDelete = effectivePermissions[PERMISSIONS.TEAMS_DELETE_ANY] || isOrgAdmin || isSuperAdmin;
      if (canDelete) {
        const checks = await Promise.all(
          teamsFromDB.map(t => checkTeamDeletableAction(t.id, t.playerIds || []))
        );
        const deletableMap: Record<string, boolean> = {};
        teamsFromDB.forEach((t, i) => { deletableMap[t.id] = checks[i].canDelete; });
        setTeamDeletable(deletableMap);
      }
    } catch (error) {
    ...
    setIsLoading(false);
  }, [activeOrganizationId, authLoading, toast, effectivePermissions, userProfile]);
