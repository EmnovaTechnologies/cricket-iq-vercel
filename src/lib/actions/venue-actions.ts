...
export default function EditVenuePage() {
  ...
  const handleDeleteVenue = async () => {
    ...
    try {
        // Safety Check 1: Is the venue used in any series?
        const seriesQuery = query(
            collection(db, 'series'),
            where('venueIds', 'array-contains', venue.id),
            limit(1)
        );
        ...
        if (!seriesSnapshot.empty) {
            ...
            toast({
                ...
                description: `Cannot delete: Venue is assigned to at least one series ("${seriesName}").`,
                variant: "destructive"
            });
            setIsDeleting(false);
            return;
        ...
        // Safety Check 2: Is the venue used in any FUTURE games?
        ...
        if (futureGamesAtVenue.length > 0) {
            const gameNames = futureGamesAtVenue.slice(0, 2).map(g => `"${g.team1} vs ${g.team2}"`).join(', ');
            toast({
                title: "Deletion Failed",
                description: `Venue is scheduled for future games (e.g., ${gameNames}). Please update these games first.`,
                variant: 'destructive',
                duration: 9000
            });
            setIsDeleting(false);
        ...
        // If all checks pass, delete the venue
