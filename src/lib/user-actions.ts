...
import { getOrganizationByIdFromDB, getPlayerByIdFromDB } from '@/lib/db';
...
export async function getAllPotentialAdmins(): Promise<UserProfile[]> {
  const { getAllUsersFromDB } = await import('@/lib/db');
  try {
...
export async function getUsersByRole(role: UserRole): Promise<UserProfile[]> {
  console.log(`[user-actions] getUsersByRole called for role: ${role}`);
  const usersQuery = query(collection(db, 'users'), where('roles', 'array-contains', role));
  ...
  console.log(`[user-actions] getUsersByRole for role '${role}' found ${users.length} users. Sorting now.`);
