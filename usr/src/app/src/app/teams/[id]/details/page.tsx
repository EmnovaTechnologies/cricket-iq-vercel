...
import { getTeamByIdFromDB, getPlayersForTeamFromDB, getAllPlayersFromDB, isPlayerAgeEligibleForTeamCategory, getAllTeamsFromDB, getOrganizationByIdFromDB, getUserProfileFromDB } from '@/lib/db';
...
import { ArrowLeft, Users, Tag, UserPlus, UserSquare2, Shield, Crosshair, Binary, UserCog, Save, Edit3, Search, Globe, Loader2, Check, ChevronsUpDown } from 'lucide-react';
...
export default function TeamDetailsPage() {
    ...
    if (teamId) {
      ...
      if (currentTeam) {
        ...
        if (currentTeam.teamManagerUids && currentTeam.teamManagerUids.length > 0) {
          const managerProfilesPromises = currentTeam.teamManagerUids.map(uid => getUserProfileFromDB(uid));
  ...
  const getSkillIcon = (skill: Player['primarySkill']) => {
    switch (skill) {
      ...
      case 'Bowling': return <Crosshair className="h-4 w-4 text-primary" />;
