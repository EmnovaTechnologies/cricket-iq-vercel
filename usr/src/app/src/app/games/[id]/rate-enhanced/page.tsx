...
import { getGameByIdFromDB, getPlayerByIdFromDB, getRatingsForGameFromDB, getUserProfileFromDB } from '@/lib/db';
        ...
        if (fetchedGame) {
          ...
          if (fetchedGame.selectorUserIds && fetchedGame.selectorUserIds.length > 0) {
            const selectorProfiles = (await Promise.all(
               fetchedGame.selectorUserIds.map(uid => getUserProfileFromDB(uid))
