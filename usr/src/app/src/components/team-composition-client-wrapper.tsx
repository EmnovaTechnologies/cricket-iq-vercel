...
import { Terminal, Info, BarChart3, Target, Shield, Loader2, Edit } from 'lucide-react';
...
interface TeamCompositionClientWrapperProps {
}

export function TeamCompositionClientWrapper({}: TeamCompositionClientWrapperProps) {
  const { userProfile, activeOrganizationId, loading: authLoading } = useAuth();
  const [seriesForDropdown, setSeriesForDropdown] = useState<Series[]>([]);
  ...
  const [selectedSeriesDetails, setSelectedSeriesDetails] = useState<Series | null>(null);
  ...
  useEffect(() => {
    if (selectedSeriesIdForTabs && !prevSeriesId) {
      setIsAnimatingTabs(true);
      const timer = setTimeout(() => setIsAnimatingTabs(false), 1500); // Match animation duration
      return () => clearTimeout(timer);
    }
    setPrevSeriesId(selectedSeriesIdForTabs);
  }, [selectedSeriesIdForTabs, prevSeriesId]);
    ...
    const fetchSeries = async () => {
      ...
      try {
        if (activeOrganizationId) {
          const seriesFromDB = await getAllSeriesFromDB('active', activeOrganizationId);
          setSeriesForDropdown(seriesFromDB);
          
          if (selectedSeriesIdForTabs && !seriesFromDB.some(s => s.id === selectedSeriesIdForTabs)) {
  ...
  const handleSeriesChange = useCallback((newSeriesId: string | null) => {
    if (newSeriesId !== selectedSeriesIdForTabs) {
      ...
      if (newSeriesId) {
        ...
        if (seriesDetails?.savedAiTeam && seriesDetails.savedAiTeam.length > 0) {
  ...
  }, [selectedSeriesIdForTabs, seriesForDropdown]);
  ...
  const handleFormSubmit = async (data: TeamCompositionFormValues) => {
    ...
    // Update tabs state immediately based on form submission
    if (seriesId !== selectedSeriesIdForTabs) {
        const seriesForTabs = seriesForDropdown.find(s => s.id === seriesId);
        setSelectedSeriesIdForTabs(seriesId);
        setSelectedSeriesNameForTabs(seriesForTabs?.name || null);
    }
      ...
      {!activeOrganizationId && !authLoading ? (
        ...
        <Tabs defaultValue="team-composition" className="w-full">
          <TabsList className={cn(
            "grid w-full h-auto",
            !selectedSeriesIdForTabs && "grid-cols-1",
            selectedSeriesIdForTabs && "grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6",
            ...
            {/* Conditionally render ranking tabs within the same list */}
            {selectedSeriesIdForTabs && (
              <>
             ...
             <Card>
              ...
              <CardContent>
                {isFetchingSeries ? (
                  ...
                  <TeamCompositionForm
                    allSeriesPassed={seriesForDropdown}
