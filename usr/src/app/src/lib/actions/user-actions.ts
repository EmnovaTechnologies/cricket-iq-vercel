...
export async function getUsersByRole(role: UserRole): Promise<UserProfile[]> {
  console.log(`[user-actions] getUsersByRole called for role: '${role}'. Fetching all users first.`);
  try {
    // Fetch all users first, then filter in code. This avoids composite index requirements.
    const allUsers = await getAllUsersFromDB();
    console.log(`[user-actions] getUsersByRole: Fetched ${allUsers.length} total users to filter from.`);
    
    const usersWithRole = allUsers.filter(user => user.roles.includes(role));
    console.log(`[user-actions] getUsersByRole for role '${role}' found ${usersWithRole.length} users.`);

    // Sorting is now handled by getAllUsersFromDB, but we can re-sort here for safety.
    ...
    // Throwing the error so the calling function can handle it.
    throw error;
  }
}
